"""API for Brafalocks bound to Home Assistant OAuth."""
import asyncio
...
class RequestFailed(Exception):
    """Exception when Brafalocks API returns an error."""
...
class TTLockAuthImplementation(
    ...
    """Brafalocks Local OAuth2 implementation."""
...
class TTLockApi:
    """Provide Brafalocks authentication tied to an OAuth2 based config entry."""

    BASE = "https://tekbrox.com/"
    ...
    async def get_locks(self) -> list[int]:
        ...
        res = await self.get("list", pageNo=1, pageSize=1000)
    ...
    async def get_lock_passage_mode_config(self, lock_id: int) -> PassageModeConfig:
        ...
        res = await self.get("get_passagemode_config", lockId=lock_id)
        return PassageModeConfig.parse_obj(res)

    async def lock(self, lock_id: int) -> bool:
        """Try to lock the lock."""
        async with GW_LOCK:
            res = await self.get("lock", lockId=lock_id)
